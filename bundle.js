...
`;
        ...
        const setSize = () => {
            ...
            const flgWh = wr < cr;
            const cw = flgWh ? cs * innerWidth : (cs * innerHeight * cr) - 20;
            const ch = !flgWh ? cs * innerHeight : ((cs * innerWidth) / cr) -20;
...
`,
    ...
    function init$1() {
        particles = [];
        color("yellow")
        particle(50,60,30,rnd(2,8))
        color("red")
        particle(50,60,30,rnd(1,2))
        color("yellow")
        particle(50,60,30,rnd(2,3))
        color("red")
        particle(50,60,30,rnd(3,4))
    }
     ...
     * @param _gameOverText
     */
    function end(_gameOverText = "  -BERNEXED- \n\n\n\n Retourne jouer\n a Catan...") {
        gameOverText = _gameOverText;
        if (isShowingTime) {
            exports.time = undefined;
        }
        particle(10,10,50,10);
        initGameOver();
    }
    /**
    ...
    function drawGameOver() {
        ...
        terminal.print(gameOverText, Math.floor((terminalSize.x - "  -Bernexed-  ".length) / 2), Math.floor((terminalSize.y-1) / 2), {color: "red"});
        terminal.draw();
    }
    function initRewind() {
        state = "rewind";
